...
class BackgroundEffect:
    ...
    def main(self, image):
        ...
        transformed_image = cv2.warpPerspective(
            image, M, (background.shape[1], background.shape[0]), borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0))


        # 将背景图像与透视变换后的图像相结合
        # 将透视变换后的图像添加到背景图的指定区域
        for i in range(background.shape[0]):
            for j in range(background.shape[1]):
                if np.all(transformed_image[i, j] != 0):
                    background[i, j] = transformed_image[i, j]
